...
class RoundedDrawable(private val sourceBitmap: Bitmap) : Drawable() {
    ...
    private fun redrawBitmapForSquareCorners(canvas: Canvas) {
        if (all(mCornersRounded)) return // no square corners
        if (cornerRadius == 0f) return // no round corners
    ...
    fun setCornerRadius(
        ...
        val radiusSet: MutableSet<Float> =
            HashSet(4)
    ...
    fun setScaleType(scaleType: ScaleType?): RoundedDrawable {
        val scaleTypeTemp = scaleType ?: ScaleType.FIT_CENTER
